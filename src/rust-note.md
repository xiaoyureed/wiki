...
- [语法](#语法)
    ...
    - [函数](#函数)
        ...
        - [高阶函数](#高阶函数)
            - [函数指针](#函数指针)
            - [禁止函数捕获外部环境中变量](#禁止函数捕获外部环境中变量)
    - [闭包](#闭包)
        - [闭包基本使用](#闭包基本使用)
        - [rust 实现闭包的原理](#rust-实现闭包的原理)
        - [捕获环境变量 and 三种闭包类型](#捕获环境变量-and-三种闭包类型)
        - [闭包作为返回值 or 参数](#闭包作为返回值-or-参数)
            - [逃逸闭包 非逃逸闭包](#逃逸闭包-非逃逸闭包)
            - [闭包作为参数](#闭包作为参数)
            - [闭包作为返回值](#闭包作为返回值)
            - [闭包参数中的生命周期](#闭包参数中的生命周期)
        - [迭代器](#迭代器)
            - [什么是迭代器](#什么是迭代器)
            - [Iterator trait](#iterator-trait)
            - [迭代器适配器 收集器 函数式风格](#迭代器适配器-收集器-函数式风格)
            - [自定义迭代器](#自定义迭代器)
            - [自定义适配器 收集器](#自定义适配器-收集器)
    - [条件循环](#条件循环)
    - [泛型](#泛型)
        - [单态化](#单态化)
        - [多重约束 加号](#多重约束-加号)
