...
https://cheats.rs/#data-structures 速查表
...
- [语法](#语法)
    ...
    - [基本数据类型](#基本数据类型)
        ...
        - [切片](#切片)
        - [str 字符串切片](#str-字符串切片)
        - [原生指针 row pointer](#原生指针-row-pointer)
        - [never 类型 感叹号](#never-类型-感叹号)
    - [字符串](#字符串)
        - [两种字符串区别](#两种字符串区别)
        - [常用方法](#常用方法)
        - [格式化](#格式化)
        - [调试打印符合类型  Debug trait Display trait](#调试打印符合类型--debug-trait-display-trait)
        - [问号操作符 多次写](#问号操作符-多次写)
    - [集合](#集合)
        - [Vec](#vec)
        - [双端队列 VecDeque](#双端队列-vecdeque)
        - [链表 Linkedlist](#链表-linkedlist)
        - [hashmap  无序哈希表](#hashmap--无序哈希表)
            - [哈希表基本使用](#哈希表基本使用)
            - [复合类型作为 key](#复合类型作为-key)
        - [BTreeMap 有序](#btreemap-有序)
        - [hashset 无序集合](#hashset-无序集合)
        - [BTreeSet 有序集合](#btreeset-有序集合)
    ...
    - [闭包](#闭包)
        - [闭包基本使用](#闭包基本使用)
        - [捕获环境变量 and 三种闭包类型](#捕获环境变量-and-三种闭包类型)
        - [闭包作为返回值 or 参数](#闭包作为返回值-or-参数)
        - [基本类型 and 复合类型被捕获时的表现](#基本类型-and-复合类型被捕获时的表现)
    - [条件循环](#条件循环)
    - [泛型](#泛型)
        - [单态化](#单态化)
        - [多重约束 加号](#多重约束-加号)
        - [默认泛型参数 and 关联类型](#默认泛型参数-and-关联类型)
        - [空约束](#空约束)
        - [turbofish 操作符 and 返回值自动推导](#turbofish-操作符-and-返回值自动推导)
    - [trait](#trait)
        - [trait 基本使用](#trait-基本使用)
        - [孤儿规则](#孤儿规则)
        - [trait 作为参数 需要 impl 前缀](#trait-作为参数-需要-impl-前缀)
        - [trait 作为 返回值 返回动态类型](#trait-作为-返回值-返回动态类型)
        - [派生 trait 和 自动推导 trait实现](#派生-trait-和-自动推导-trait实现)
        - [运算符重载](#运算符重载)
        - [trait 继承](#trait-继承)
        - [静态分发 动态分发](#静态分发-动态分发)
        - [抽象类型](#抽象类型)
            - [trait对象](#trait对象)
            - [trait对象安全问题 Sized trait](#trait对象安全问题-sized-trait)
            - [impl trait](#impl-trait)
        - [trait的类型转换](#trait的类型转换)
        - [From trait Into trait](#from-trait-into-trait)
        - [DerefMut和 Deref](#derefmut和-deref)
        - [AsRef trait AsMut trait](#asref-trait-asmut-trait)
        - [Borrow trait](#borrow-trait)
        - [Debug trait](#debug-trait)
        - [Drop trait 资源释放](#drop-trait-资源释放)
        - [Clone trait](#clone-trait)
        - [标签 trait](#标签-trait)
            - [Send trait 和 Sync trait](#send-trait-和-sync-trait)
            - [Copy trait](#copy-trait)
            - [Sized trait  和 Unsized trait 和 动态类型](#sized-trait--和-unsized-trait-和-动态类型)
        - [Default trait](#default-trait)
    - [元组](#元组)
    - [结构体](#结构体)
        - [结构体基本使用](#结构体基本使用)
        - [元组结构体 and 单元结构体](#元组结构体-and-单元结构体)
        - [结构体方法](#结构体方法)
        - [new type 模式 和 类型别名 Self别名](#new-type-模式-和-类型别名-self别名)
    - [枚举](#枚举)
        - [枚举基本使用](#枚举基本使用)
        - [Option](#option)
            - [match 解开](#match-解开)
            - [option enum 常用方法](#option-enum-常用方法)
        - [c 风格的枚举](#c-风格的枚举)
        - [实例 创建链表](#实例-创建链表)
    - [match 模式匹配](#match-模式匹配)
        - [模式匹配简单使用](#模式匹配简单使用)
        - [if lef while let 语法糖](#if-lef-while-let-语法糖)
    - [错误处理](#错误处理)
        - [panic 宏](#panic-宏)
        - [Result 问号操作符](#result-问号操作符)
        - [错误处理进化过程](#错误处理进化过程)
        - [错误装箱](#错误装箱)
    - [io](#io)
        - [命令行参数](#命令行参数)
...
- [工程管理 模块](#工程管理-模块)
    ...
    - [build.rs](#buildrs)
    - [项目管理案例](#项目管理案例)
        - [在 bin 项目内新建 lib](#在-bin-项目内新建-lib)
        - [执行子目录中的 module](#执行子目录中的-module)
...
- 所有权系统几乎解决了野指针, 强大的类型系统

- zero cost abstract (编译期间即可编译成机器码, 运行时没有开销), 没有 runtime 和 gc, 和 c 无缝交互
...
1. VSCode + rust 插件 + native debug 插件 + TabNine 提示插件 + better toml plugin [ + crates 方便包管理的插件]

1. vscode + rust-analyzer plugin + CodeLLDB plugin (debug) + even better toml plugin (better toml) 格式化 toml [+ crates]
...
use ferris_says::say;// 导入模块 // 支持 as , 如 use xxx as yyy
use std::io::{stdout, BufWriter}; 
...
// 常量, 静态变量, 一般在全局声明, 不在函数里声明
    // 需要手动指定类型
    // 区别:
    // - 静态变量有固定的内存地址 (分配在静态存储区); 常量在被用到时会被复制过去, 用不到内存地址
    // - 静态变量可以是可变的 (用 mut 修饰), 修改需要在 unsafe 中
...
分为 不可变 row pointer 和 可变 row pointer
...
### 调试打印符合类型  Debug trait Display trait
...
是一种 同时具有 队列(先进先出)和战 (后进先出)性质的数据 结构 。 双端队列中的元素可 以从两端弹出，插入和删除操作被限定 在队列的两端进行 。
...
因为是双向列表， 所以提供了 push back 和 push front 两类方法，方便操作 此链表。也提供了 append 方法 ， 可
以用来连接两个链表
...
fn hash_table() {
    ...
    let mut map = HashMap::new();//获得一个带有默认初始容量的 HashMap
    ...
    //不存在才插入, 设定值为 red, 否则将跳过, 一般的插入则是有则覆盖旧的值
    map.entry("color").or_insert("red");// , 返回 entry 的 value 的 reference
...
fn iter_demo() {
    ...
    *ele_first = 100; // 需要写操作, 先解引用
    ...
    let scaled1: Vec<_> = v.iter_mut().map(|&mut x| x* 10).collect(); //等效, 不改变原始 v
    println!("scaled1 = {:?}", scaled1);
    println!("v = {:?}",v);//不变
    let plused: Vec<_> = scaled.into_iter().map(|x| x +1).collect();
...
```rust
    ...
    // &str -> String
    let string = "hello".to_owned(); //最常用, 吧 "hello"从栈转移到堆
...
/// 发散函数: 返回值类型为 "!" (never type), 标识函数永远不会返回, 比如 函数包含死循环, 或者 panic
///     发散函数可以作为任何类型. 应用:可以在正常有返回值函数中 panic, 在if 判断中 panic
...
/// 闭包中可以捕获外部环境中的变量
    ...
    // - FnOnce 获取变量所有权, 闭包需要将该变量所有权移动到闭包内, 使得外部的这个变量失效; once 表示变量的所有权只能获取一次
    //              由于所有所有闭包都可以至少调用一次, 所以所有闭包都实现了 FnOnce (闭包涉及到捕获环境变量才会自动成为 FnOnce)
    //      (加了 move)
    // - FnMut 获取可变的借用值, 所以可以改变这个变量 
    //          没有移动外部变量所有权进内部的闭包, 都实现了 FnMut
    //      (没加 move, 内部有写操作)
    // - Fn     闭包从外部获取不可变的借用值 
    //          不需要对外部变量进行写操作的闭包实现了 Fn
    //      (没加 move, 内部无写操作)
    // 
    //
    let mut i = 1;
    // 修改了 可变变量, 如果调用, 函数需要声明为 可变
    let mut func =  || {
        i+=1;
    };
    func();
...
// 作为参数
//必须通过泛型, 如果直接在形参类型中指定 Fn, 报错
fn fu<T: Fn() -> u8>(f: T) -> u8 {
    f()
}
    ...
    // 
    //返回类型声明: 
    // - 目前 Rust 只支持返回具体（非泛型）的 类型。按照定义，匿名的闭包的类型是未知的，所以只有使用impl Trait才能返回一个闭包
        // - Fn：和前面的一样
        // - FnMut：和前面的一样
...
imp! Trait代表静 态分发 ， dyn Trait 代表动态分发 。
...
```rs
// 实现了最常用 的 Debug trait， 就可 以拥有在 println!宏语句中 使用{ :?}格式进行打印的行为，
...
实现 clone, colon_from 方法

如果一个类型是 Copy的， 它的clone方法仅d需要返回*self即可 
...
#### Sized trait  和 Unsized trait 和 动态类型
...
### new type 模式 和 类型别名 Self别名

当一个元组结构体只有一个字段的时候，我们称之为 New Type 模式

new type 惯用法: 为不同种类的数据分别定义新的类型 (这是为了能保证在编译时，编译期间即可发现错误。), 包装了一层


```rust

// newtype 模式: 有时候 , 我们希望给标准库中的结构实现标库中的 trait, 如给 Vector 实现 Display trait.
    //这明显不行, 可以通过给 Vector 包装一层来达到目的
...
```rust
    ...
    // 更好的写法: 使用 ? 号
    // ?号 是 “要么 unwrap 要么 return Err(From::from(err))”。From::from 是 不同类型间的转换工具，
    // 也就是说，如果在错误能够转换成返回类型的地方使用 ?，它就 会自动转换成返回类型。
...
https://users.rust-lang.org/t/video-series-build-a-smart-bookmarking-tool-with-rust-and-rocket/47601 rocket 实例
